...
The Article Collection Funnel fetches configured sources, scrapes articles, rejects source mismatches, dedupes URLs, records Source Run diagnostics, persists candidate URL history, and yields fresh article candidates. When enabled, the optional Translation stage consumes those candidates before global story clustering.

## Source Catalog
The Source Catalog is `config/sources.yaml`. It owns source records, YAML layout, and authoritative human-declared source-language tags. The translation stage consumes those tags but never guesses from article text or automatically retags sources; source removal remains catalog-owned.
