...
<body>
  ...
  <script>
      ...
      $("scheduleMount").innerHTML = `
        ...
        <div class="stack">
          <section class="panel">
            ...
            <p class="muted" style="margin:10px 0 0">Exactly one <code>HH:MM</code> in local time via <code>launchd</code> (<code>StartCalendarInterval</code>, macOS only) — <code>HH:MM (local time, once daily)</code> for the next firing; no weekly recurrence or cron expressions. If the machine is asleep, off, or not logged in at <code>HH:MM</code>, the run does not fire and nothing is queued — the next eligible daily window runs instead, or run <code>uv run news schedule run</code> now. Credentials are never stored in schedule state or the launchd plist.</p>
            <p class="muted">Model and hardware: scheduled runs inherit the preset/model; larger models need more RAM/VRAM. Default <code>gemma-4-12b-it-4bit</code> (<code>mlx-vlm</code>, 256K) wants 32GB+ RAM; <code>qwen3-8b-4bit</code> (<code>mlx-lm</code>, 40K verified) is smaller while <code>qwen3-14b-4bit</code> is host-sensitive (64GB-class); <code>qwythos-9b-*</code> GGUFs need <code>llama-server</code> (<code>llama.cpp</code>). See <code>config/model_catalog.yaml</code> and each Hugging Face model page’s Hardware Compatibility panel. A mismatched <code>NEWS_MODEL_BACKEND</code> fails fast; if an <code>mlx</code> run OOMs, re-enable with a smaller alias (e.g. <code>gemma-e2b-tiny</code> with <code>NEWS_MODEL_BACKEND=mlx-lm</code>).</p>
            <p class="muted">Failure semantics: report and delivery are independent — <code>skipped: not_configured</code> (no sender/recipient/SMTP), <code>skipped: user_disabled</code> (disabled or all-paused), or <code>failed</code> never hide the report; delivery phase/status plus accepted/rejected lists live in <code>latest_run_details.json</code> and history. Retry is the next daily firing or <code>uv run news schedule run</code>. Status shows <code>enabled</code>, <code>launchd loaded/not-loaded/unavailable</code>, and last run id/time/run/report/delivery without secrets.</p>
          </section>
          <section class="panel">
            <p class="eyebrow">Status</p>
    ...
    function renderModelBackendHint(requiredBackend = "") {
      ...
      const current = effectiveModelBackend();
      if (!required || !current) {
    ...
    function refreshHuggingFaceUseButtons() {
      const backendExternal = effectiveModelBackend() === "external";
