...
class ConfigHelperTests(unittest.TestCase):
    ...
    def test_registry_ui_location_families_are_annotated_exactly(self) -> None:
        ...
        # and the 30 sampling fields for the five actual LLM tasks. The
        # translation task (issue #172) has no dedicated panel yet, so its
        # knobs stay raw Advanced overrides alongside its task selector.
        for task, _, _ in config_module.MODEL_TASK_KNOB_SPECS:
            suffix = task.upper()
            dedicated = task != config_module.MODEL_TASK_TRANSLATION
            expected_location = (
        ...
        llm_sampling_tasks = {spec[0] for spec in config_module.MODEL_TASK_KNOB_SPECS} - {
            config_module.MODEL_TASK_TRANSLATION
        }
