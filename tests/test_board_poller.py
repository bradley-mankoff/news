...
class FetchRunsByMessageTest(unittest.TestCase):
    """Newest-run-per-message selection and gh-failure handling."""

    def test_keeps_newest_run_per_message(self) -> None:
...
class MergePrToBaseTests(unittest.TestCase):
    """Auto-close reopen guard on develop merges (issue #22)."""

    CFG = {"repo": "bradley-mankoff/news"}

    def test_reopens_auto_closed_issue(self) -> None:
        with patch("automation.board_poller.gh") as gh, \
            ...
            ok, note = poller.merge_pr_to_base(
                self.CFG, {}, {"number": 1, "state": "OPEN", "baseRefName": "develop"},
                "develop", 7)
        self.assertTrue(ok)
        self.assertIn("reopened", note)
        sleep.assert_called_once_with(6)

    def test_skips_reopen_when_issue_stays_open(self) -> None:
        with patch("automation.board_poller.gh") as gh, \
             patch("automation.board_poller.time.sleep") as sleep:
            gh.side_effect = [
                subprocess.CompletedProcess([], 0),           # pr ready
    ...
    def test_short_circuits_when_already_merged(self) -> None:
        with patch("automation.board_poller.gh") as gh:
            ok, note = poller.merge_pr_to_base(
                self.CFG, {}, {"number": 1, "state": "MERGED", "baseRefName": "develop"},
                "develop", None)
        self.assertTrue(ok)
...
class ShipPrTests(unittest.TestCase):
    """Ship-PR find-or-create semantics (issue #22)."""
    ...
    def test_find_or_create_ship_pr_reuses_existing_open_pr(self) -> None:
        ...
        with patch("automation.board_poller.gh", return_value=r) as gh:
            pr = poller.find_or_create_ship_pr(self.CFG, {}, "archon/task-issue-7",
                                               "Ship #7", 7, "main")
    ...
    def test_find_or_create_ship_pr_creates_when_missing(self) -> None:
        ...
        created = subprocess.CompletedProcess(
            [], 0, stdout="https://github.com/o/r/pull/9")
