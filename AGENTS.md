...
- Archon execution: archon-pi build, home at `~/.local/share/archon-pi/archon-home/`; routine nodes use Pi/OpenCode Zen Go and rigorous nodes use Pi's OpenAI Codex OAuth backend, both at max effort. The current 17-workflow inventory and routing split are in `docs/archon-workflows.md`.
...
- **Dependencies:** a `Depends on: #N` line in the issue body (one line; `#42, #57` for several) gates dispatch. An issue dragged to `Todo` with an unsatisfied dependency (referenced issue not in `Done`) is moved to `Blocked` with a comment instead of dispatching; it returns to `Todo` (and dispatches) automatically when its deps ship. A dependency that closes without shipping posts a re-scope notice on each dependent. Dependencies must be board items in this repo. Fill `Depends on` before dragging to `Todo`; the issue templates include the field. `Blocked` is exclusively dependency gating; `Needs Input` is exclusively NEEDS INPUT questions.
- **Slicing convention (parallel-safe issues):** slice issues as tracer bullets — narrow, end-to-end vertical slices (schema → API → UI → tests), each demoable on its own and sized to one context window — and declare file/area ownership in the plan. Two issues may run in parallel only when their planned file sets are disjoint; overlapping ownership requires the later one to `Depends on` the earlier. Any set of issues with satisfied dependencies can be triggered together safely. Issue templates carry binary acceptance criteria, an `Out of scope` section (the only legitimate source of deferrals), and `Depends on`.
...
- **Completion records:** implementation workflows post a structured comment on the issue after each run: `## What shipped` (substance, not run narration), `## Decisions` (choice + why only), `## Acceptance criteria` (each criterion with cited evidence). The record also carries a `## Deferred work` section (one bullet per deferred item). The workflow judges each item: a size bar (only independently schedulable deliverables spawn; chores/tests/docs are skipped and preserved in the record) and a dedupe pass over open/closed issue titles + initial bodies + repo context, stamping `**Links to:** #N` / `**Supersedes:** #N` / `**Skip:**`; the poller executes mechanically (link, create a Backlog issue, or skip) and comments the linkage. Deferral prose without the section posts a verification comment instead. When closing an issue without shipping, leave a comment with what was tried and why it was abandoned.
...
- When the dispatched run completes, the poller marks the PR ready and **merges it into `develop`**, then moves the issue to `Ready for Review` — that is the signal to test the integration branch locally.
- **Develop conflicts auto-fix:** a develop PR that cannot merge is handled without a human, mirroring the ship lane: the poller first merges `develop` into the branch via the GitHub merge API; on real conflicts it dispatches `archon-fix-develop-conflicts` (merges develop, resolves both intents, validates, pushes, comments), once per conflict episode. If the fix run finishes and the PR is still conflicting, the poller comments on the issue asking for manual help (merge develop into the branch; the poller merges automatically once it is mergeable). Implementation workflows also sync with develop before opening the PR, which prevents most collisions.
...
- Make no further changes. The run ends; the poller moves the issue to `Needs Input` (no develop merge).
