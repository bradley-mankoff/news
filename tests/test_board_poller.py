...
class FillConcurrencyGapTest(unittest.TestCase):
    ...
    def test_poll_promotes_backlog_then_dispatches_on_next_poll(self):
        ...
        with (
            ...
            # The first poll establishes the snapshot and fills the available
            # slots, but the existing transition loop does not dispatch yet.
            ...
            self.assertEqual(
                [state[item["id"]]["issue_number"] for item in items], [1, 2]
            )

            cycle_adapter.poll(cfg, {}, state)

        # On the next poll the persisted Backlog -> Todo transitions reach the
        # normal dispatch path, rather than requiring another board edit.
